...
ENABLE_COVERAGE = IN_CI or getenv('KUDZU_TEST_COVERAGE') is not None
SKIP_EXAMPLES   = getenv('KUDZU_TEST_NO_EXAMPLES') is not None
SKIP_FORMAL     = getenv('KUDZU_TEST_NO_FORMAL') is not None
...
def test(session: Session) -> None:
	...
	with session.chdir(OUTPUT_DIR):
		...
		if SKIP_FORMAL or len(session.posargs) > 0:
			session.log('Skipping formal...')
		else:
			session.log('Running formal...')
...
def build_docset(session: Session) -> None:
	...
	# Get the Kudzu version
	kudzu_version: str = session.run(
		'python', '-c', 'import kudzu;print(kudzu.__version__)',
		silent = True
	) # type: ignore

	with session.chdir(BUILD_DIR):
		# If the docset is already built, shred it because `doc2dash` won't overwrite it
		if (BUILD_DIR / 'Kudzu.docset').exists():
			rmtree(BUILD_DIR / 'Kudzu.docset')

		# Build the docset
		session.run(
			'doc2dash', '-n', 'Kudzu', '-j', '--full-text-search', 'on', str(DOCS_DIR)
		)

		# Compress it
		make_archive(f'kudzu-{kudzu_version.strip()}-docset', 'zip', BUILD_DIR, 'Kudzu.docset')
...
def dist_docs(session: Session) -> None:
	...
	with session.chdir(BUILD_DIR):
		make_archive(f'kudzu-{kudzu_version.strip()}-docs', 'zip', BUILD_DIR, 'docs')
...
def typecheck_mypy(session: Session) -> None:
	...
	session.run(
		...
		'-p', 'kudzu', '--html-report', str(OUTPUT_DIR.resolve())
	)
...
def lint(session: Session) -> None:
	...
	session.run(
		...
		'./kudzu', './tests', './examples', './docs'
	)
	session.run('ruff', 'check', './kudzu', './tests', './examples', './docs')
